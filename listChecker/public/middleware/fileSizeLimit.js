function fileSizeLimit (req, res, next){
    ...
    if (filesOverLimit.length){
        const fileSize = files[Object.keys(files)].size/1024/1024;
        return res.status(413).json({ status:'error', message:`File over limit of 5MB: '${filesOverLimit[0]}' (${fileSize.toFixed(0)} MB)`});
    }
